...
发布版本统一使用 `0.8.1`，tag 为 `v0.8.1`。

| 渠道 | 产物 |
|---|---|
| GitHub / Gitee Release | `spark-store-tui-source-0.8.1.tar.gz`、每个架构的 `.deb`、RPM / SRPM |
| Debian | `spark-store-tui_0.8.1-1_amd64.deb` 或 `..._arm64.deb` |
| RPM | `spark-store-tui-0.8.1-1.<arch>.rpm` |
...
cp spark-store-tui-source-0.8.1.tar.gz ~/rpmbuild/SOURCES/
...
git commit -m "spark-store-tui 0.8.1"
git push
```
